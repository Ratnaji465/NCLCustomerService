...
class CreateClientProjectActivity : NetworkChangeListenerActivity(), RetrofitResponseListener {
            ...
            save.setOnClickListener {
                ...
                if (isValidate()) {
                    if (tvShortage.text?.toString().equals("Select")) {
                        Toast.makeText(this@CreateClientProjectActivity, "Please Select Shortage", Toast.LENGTH_SHORT).show()
                        return@setOnClickListener
                    }else if (tvWorkCompletion.text?.toString().equals("Select")) {
                        Toast.makeText(this@CreateClientProjectActivity, "Please Select Work Completion", Toast.LENGTH_SHORT).show()
                        return@setOnClickListener
                    }else if (tvWorkStatusSelect.text?.toString().equals("Select")) {
                        Toast.makeText(this@CreateClientProjectActivity, "Please Select Work Status", Toast.LENGTH_SHORT).show()
                        return@setOnClickListener
                    }
                    callApi()
                }
    ...
    private fun uploadImage(request: ClientProject) {
        ...
        if (wc_certificate_file != null && wc_certificate_file!!.length() > 5) {
            muPartList.add(prepareFilePart("work_completion_certificate_image_path[]", Uri.fromFile(wc_certificate_file), wc_certificate_file!!)!!)
        }
        Common.Log.i("Request obj " + request.toString())
        if (muPartList.size > 0) {
            val fileParts = muPartList.toTypedArray()
            val abc = MyApplication.getInstance().apiInterface.uploadPaymentCollection(Constants.API, fileParts, request)
            abc.enqueue(object : Callback<ResponseBody?> {
                override fun onResponse(call: Call<ResponseBody?>, response: Response<ResponseBody?>) {
                    if (response.body() == null) {
                        Toast.makeText(this@CreateClientProjectActivity, "Intenal Server Error", Toast.LENGTH_SHORT).show()
                        progressD!!.dismiss()
                        return
                    }
                    Toast.makeText(this@CreateClientProjectActivity, "New contact inserted successfully.", Toast.LENGTH_SHORT).show()
                    var apiResponseController: ApiResponseController? = null
                    try {
                        apiResponseController = Gson().fromJson(response.body()!!.string(), ApiResponseController::class.java)
                        val clientProjectResVO = Common.getSpecificDataObject(apiResponseController.result, ClientProjectResponse::class.java)
                        if (clientProjectResVO != null) {
                            val clientProjectList: List<ClientProject> = clientProjectResVO.clientProjects
